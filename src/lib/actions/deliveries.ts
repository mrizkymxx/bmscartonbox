...
export async function getDeliveries(): Promise<Delivery[]> {
  try {
    ...
    const deliveries: Delivery[] = snapshot.docs.map((doc: QueryDocumentSnapshot) => {
      const data = doc.data();
      return {
        id: doc.id,
      ...
      po.items.forEach((item) => {
        ...
        if (availableToShip > 0) {
          items.push({
            ...
            orderDate: (po.orderDate as any) instanceof Timestamp ? (po.orderDate as any).toDate().toISOString() : po.orderDate,
